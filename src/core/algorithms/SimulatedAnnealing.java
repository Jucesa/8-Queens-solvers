package core.algorithms;

import core.NQueensSolver;

import java.util.ArrayList;
import java.util.List;
import java.util.function.BiConsumer;

public class SimulatedAnnealing extends NQueensSolver {
    private final int k;
    private final double temp;
    private final double cooling;

    public SimulatedAnnealing(int k, double temp, double cooling) {
        this.k = k; this.temp = temp; this.cooling = cooling;
    }

    @Override
    public String getName() { return "Simulated Annealing (K=" + k + ")"; }

    @Override
    public int solve(boolean[] board, BiConsumer<boolean[], Integer> onStep) {
        int currentAttacks = calcularAtaques(board);
        double currentTemp = this.temp;
        onStep.accept(board, 0);

        for (int i = 1; i <= k; i++) {
            if (currentAttacks == 0) break;

            List<Integer> queens = new ArrayList<>();
            List<Integer> empty = new ArrayList<>();
            for (int idx = 0; idx < TOTAL_SQUARES; idx++) {
                if (board[idx]) queens.add(idx);
                else empty.add(idx);
            }

            int qIdx = queens.get(rand.nextInt(queens.size()));
            int destIdx = empty.get(rand.nextInt(empty.size()));

            board[qIdx] = false; board[destIdx] = true;
            int newAttacks = calcularAtaques(board);
            int delta = newAttacks - currentAttacks;

            if (delta < 0) {
                currentAttacks = newAttacks;
                onStep.accept(board, i);
            } else {
                double probability = Math.exp(-delta / currentTemp);
                if (rand.nextDouble() < probability) {
                    currentAttacks = newAttacks;
                    onStep.accept(board, i);
                } else {
                    board[qIdx] = true; board[destIdx] = false;
                }
            }
            currentTemp *= cooling;
        }
        return currentAttacks;
    }
}
...
public class Selection {
    ...
    public static RNA tournamentN(RNA[] population, int tournamentSize) {
        Random random = new Random();

        RNA best = population[random.nextInt(population.length)];

        for (int i = 1; i < tournamentSize; i++) {
            RNA contender = population[random.nextInt(population.length)];
